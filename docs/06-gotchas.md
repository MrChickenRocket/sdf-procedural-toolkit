...
| Surface **seam/ridge** near where a subtract cutter ends *inside* the solid (a blind hole) | The cutter's end-cap leaves a thin domed cavity the mesher can't resolve at practical cell sizes; the artifact telegraphs to the nearby outer surface | **Extend the cutter clean THROUGH** both surfaces (start it outside, end it outside) instead of terminating it inside. A through-cut meshes cleanly at coarse cells; a blind cut needs impractically fine cells. (Confirmed on the soldier's gun-barrel bore.) |
